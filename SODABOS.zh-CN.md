...
<p align="center">
  <em>别再把 AI 当助手用了。让你的业务围绕它来运转。</em>
</p>

<p align="center">
  Agent 不好用，原因有二：<strong>感知层太小</strong>（它只看到一个聊天窗口），<br/>
  以及我们用错了方式 — 把它当坐在旁边的助手，而不是<strong>所有人围绕它来协作的中心</strong>。<br/>
  SodaBOS 同时解决这两个问题。它给每个 Agent — Hermes、OpenClaw、Claude Code、你自己的 —<br/>
  一个带有完整业务上下文的共享大脑。也给你的团队在同一张桌子上留了座位。<br/>
  <strong>平台会思考、会做梦、会学习、会进化。你的 Agent 和你的人一起成长。</strong>
</p>
...
<p align="center">
  ...
  <a href="#看看效果">截图</a> ·
  <a href="#架构">架构</a> ·
  <a href="#sodabos-与-agent-框架的关系">vs. Agent</a> ·
...
**问题一：感知层太小。** 今天的 Agent 只看到一个聊天窗口和几个工具调用。它看不到你的项目历史、团队方法论、过往的错误、正在进行的讨论、或者一个需求背后的业务上下文。一个没有上下文的 Agent 只能做浅层决策 — 所以你得一直盯着它，它就变成了一个高级自动补全。

**问题二：我们用反了。** 所有人都把 AI 当助手 — 一个坐在你旁边帮你干活的同事。但在真实业务里，更好的架构恰恰相反：**AI 是中心，团队围绕它协作。** AI 持有记忆、方法论、上下文。人和其他 Agent 接入进来，贡献各自的专业能力，整个系统一起进化。

SodaBOS 就是从这个洞察出发设计的。它不是又一个 Agent — 它是那个让每个 Agent（和每个人）都变得强大得多的**共享大脑**。
...
## SodaBOS 与 Agent 框架的关系

大多数开源 AI 项目是 **Agent** — 它们自己就是大脑。SodaBOS 不是 Agent，它是 **Agent 接入的平台**。这是根本性的架构区别。

```
Agent 框架（Hermes、OpenClaw、CrewAI、AutoGPT……）：
  ┌──────────────────────┐
  │  Agent               │   Agent 本身就是产品。
  │  ├── 记忆            │   它思考、行动、记忆。
  │  ├── 工具            │   人类通过聊天交互。
...
SodaBOS（AI 中台）：
  ┌──────────────────────────────────────────┐
  │  SodaBOS 平台                             │   平台是产品。
  │  ├── 持久记忆 & 方法论                     │   多个 Agent 通过 MCP 接入。
  │  ├── 认知循环（9 阶段）                    │   人类通过完整 GUI 工作。
  │  ├── 做梦系统                              │   所有人共享同一个大脑。
  │  ├── 圆桌讨论                              │
  │  ├── GUI ←→ MCP 双接口                    │
  │  └── 知识库、经验教训、检查点               │
...
| 能力 | SodaBOS | Hermes Agent | OpenClaw | CrewAI | AutoGPT | LangGraph |
|---|---|---|---|---|---|---|
| **定位** | 平台 / 中台 | 自主 Agent | 守护进程 Agent | 多 Agent 编排 | 任务分解器 | 图编排框架 |
| **多 Agent 共享状态** | ✅ 核心设计 | ❌ 单 Agent | ❌ 单 Agent | ⚠️ 一组 Crew 内 | ❌ | ⚠️ 一张图内 |
| **人类 GUI（不是聊天）** | ✅ 完整 SaaS 界面 | ❌ 聊天频道 | ❌ 聊天频道 | ⚠️ Studio（配置） | ❌ | ❌ |
| **MCP 服务端（Agent 接入）** | ✅ 25+ 工具 | ✅ 可作为 MCP 服务 | ✅ 连接 MCP | ❌ | ❌ | ❌ |
| **持久记忆** | ✅ 统一存储 | ✅ SQLite | ✅ Markdown 文件 | ❌ | ⚠️ 向量库 | ✅ |
| **做梦 / 反思** | ✅ 4 阶段系统 | ⚠️ 自我改进 | ❌ | ❌ | ❌ | ❌ |
| **矛盾检测** | ✅ 交叉校验信念 | ❌ | ❌ | ❌ | ❌ | ❌ |
| **方法论提炼** | ✅ 可复用原则 | ❌ | ❌ | ❌ | ❌ | ❌ |
| **主动提问** | ✅ 发布到圆桌 | ❌ | ❌ | ❌ | ❌ | ❌ |
| **领域无关** | ✅ 替换实体层 | ⚠️ 通用 | ⚠️ 通用 | ⚠️ 任务特定 | ⚠️ 任务特定 | ✅ 底层 |

### 它们不是竞品 — 它们是客户端

Hermes Agent 和 OpenClaw 可以**通过 MCP 作为客户端连接 SodaBOS**。CrewAI 的 Crew 可以调用 SodaBOS 工具。LangGraph 的节点可以查询 SodaBOS 知识库。关系是：

```
Hermes Agent  ──MCP──→  SodaBOS  ←──GUI──  产品经理
OpenClaw      ──MCP──→  （共享记忆、      ←──GUI──  团队负责人
Claude Code   ──MCP──→   方法论、         ←──GUI──  设计师
自建 Bot      ──API──→   知识库）         ←──GUI──  数据分析师
```

SodaBOS 不替代你的 Agent — 它给你的 Agent 一个**业务大脑**来思考、一个**团队**来协作、一份**记忆**来跨越任何单次会话。
